...
export class GatlingModel{
  ...
  reset(){this.inventory=new WeaponInventory();this.equipped=[false,false];this.cooldown=[0,0];this.flash=[0,0];this.shieldTime=[0,0];this.slashes=[];this.grenades=[];this.lasers=[null,null];this.transform=[0,0];this.bullets=[];this.rockets=[];this.explosions=[];this.sparks=[];this.room=-1;this.active=false;this.ticks=0;this.shots=[0,0];this.hits=[0,0];this.targetsSpawned=false;this.notice='Face an item: X to pick up. S / Q to fire.';this.noticeTime=360;this.pickupSerial=0;this.booms=0;this.terrain?.reset();}
  get pickups(){return this.inventory.ground.filter(g=>g.room===this.room&&g.type);}
  get shields(){return this.shieldTime??=[0,0];}
  player(ram,i){const b=0x100+i*0x80;return {x:word(ram,b+0x11),y:word(ram,b+0x14),z:ram[b+0x17],direction:(ram[b+0x47]>>1)&3,visible:!!ram[b]&&!!ram[b+1],canAct:(ram[b]===1||ram[b]===3)&&ram[b+2]===2&&ram[b+3]===0};}
  spawnTargets(ram){
  ...
  slash(ram,p,owner){
    const [dx,dy]=DIRECTIONS[p.direction];this.slashes??=[];this.slashes.push({x:p.x,y:p.y-10,direction:p.direction,life:12});
    for(let b=0x200;b<0x980;b+=0x50){const x=word(ram,b+0x11)-p.x,y=word(ram,b+0x14)-p.y;
      if(Math.hypot(x,y)<38&&x*dx+y*dy>-5&&defeatPirate(ram,b,p.direction))this.hits[owner]++;
    }
  }
  tick(ram,masks){
    ...
    this.slashes=(this.slashes||[]).filter(s=>--s.life>0);this.grenades??=[];
    for(let i=0;i<2;i++){
      this.shields[i]=Math.max(0,this.shields[i]-1);
      ...
      if(firing&&weapon==='mech'){
        ...
        if(!this.cooldown[i]){this.fireLaser(ram,this.lasers[i]);this.cooldown[i]=6;this.shots[i]++;this.inventory.spend(ram,i);if(this.inventory.held[i]!=='mech')this.lasers[i]=null;}
      }else if(firing&&!this.cooldown[i]){
        const [dx,dy]=DIRECTIONS[p.direction];
        const projectile={x:p.x+dx*12,y:p.y+dy*12,direction:p.direction,owner:i,age:0,type:weapon,hit:new Set()};
        if(weapon==='sword')this.slash(ram,p,i);
        else if(weapon==='shield')this.shields[i]=SHIELD_TIME;
        else if(weapon==='grenade')this.grenades.push(projectile);
        else (weapon==='rocket'?this.rockets:this.bullets).push(projectile);
        this.cooldown[i]=WEAPONS[weapon]?.delay||4;this.flash[i]=weapon==='rocket'?8:3;this.shots[i]++;this.inventory.spend(ram,i);
      }
    }
    ...
    const advance=(b,rocket)=>{
      ...
      for(let step=0;step<(rocket?2:b.type==='sniper'?8:4);step++){
        b.x+=dx*2;b.y+=dy*2;
        if(b.x<8||b.x>247||b.y<18||b.y>215||b.age>(rocket?60:40)){if(rocket)this.explode(ram,b);return false;}
        for(let slot=0x200;slot<0x980;slot+=0x50){
          if(!ram[slot]||ram[slot+0xa]!==0xc||!ram[slot+0x1c]||b.hit?.has(slot))continue;
          if(Math.abs(b.x-word(ram,slot+0x11))<10&&Math.abs(b.y-word(ram,slot+0x14))<10&&ram[slot+0x17]<16){
            ...
            b.hit?.add(slot);if(b.type!=='sniper')return false;
          }
        }
    ...
    this.grenades=this.grenades.filter(g=>{g.age++;const [dx,dy]=DIRECTIONS[g.direction];if(g.age<30){g.x=Math.max(10,Math.min(245,g.x+dx*2));g.y=Math.max(30,Math.min(205,g.y+dy*2));}if(g.age>=45){this.explode(ram,g);return false;}return true;});
  }
}
